...
def _print_with_flags(text):
    print(text)
    flags=find_flags(text or '')
...
def register(sub):
    ...
    z=sp.add_parser('dns',help='Query common DNS records',description='Use for domain/DNS challenges to inspect A/AAAA/MX/TXT/CNAME-style clues.'); z.add_argument('domain'); z.set_defaults(fn=lambda a: _print_with_flags(lookup(a.domain)))
    z=sp.add_parser('connect',help='TCP connect and read initial banner',description='Use for nc-style CTF services to see the server greeting/banner.'); z.add_argument('host'); z.add_argument('port',type=int); z.set_defaults(fn=lambda a: _print_with_flags(connect(a.host,a.port)))
def _scan(a):
