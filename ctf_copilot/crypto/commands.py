...
def register(sub):
    ...
    z=sp.add_parser('vigenere',help='Decrypt with a known key or rank likely keys',description='Use when the challenge explicitly suggests Vigenere. Automatic candidates remain heuristic.'); z.add_argument('value'); z.add_argument('--key'); z.add_argument('--max-key-length',type=int,default=20); z.set_defaults(fn=_vigenere)
...
def _vigenere(args):
    if args.key:
        ...
        return
    rows=crack_vigenere(args.value,max(2,min(args.max_key_length,40)))
    if not rows:
