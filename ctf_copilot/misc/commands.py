...
def _print_decoded(text):
    print(text)
    flags=find_flags(str(text or ''))
...
def register(sub):
    ...
    z=sp.add_parser('morse',help='Decode Morse code',description='Use on dot/dash-separated Morse text.'); z.add_argument('value'); z.set_defaults(fn=lambda a: _print_decoded(decode(a.value)))
