...
def bitflip(url: str, cookie: str, cookie_name: str='auth_name', max_attempts: int=256, progress=None) -> str:
    ...
    for byte_index in range(len(raw)):
        for bit in range(8):
            ...
            attempts+=1
            if progress and (attempts==1 or attempts%25==0): progress(f'  Tested {attempts}/{max_attempts} cookie changes...')
